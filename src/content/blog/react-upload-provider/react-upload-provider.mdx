---
title: 'A simple way to improve your file uploads in React with progress, cancelation and better accesibility'
slug: improve-react-uploads-with-progress-cancelation-accessibility
...
By defining the **UploadFunction** and **UploadResult** generic types, you can wrap the function you already use to upload files to return type safe values with an extra error variable to check for user cancelation or network problems.
Looking at the context data, we are defining a boolean to tell the childrens if an upload is already in progress and also an **startUpload** function which is the core of the functionality.
...
Within the `ChatInput` component, you might handle file uploads as follows, utilizing the `useUpload` hook to access the upload context:
